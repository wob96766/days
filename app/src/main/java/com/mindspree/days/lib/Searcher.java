...
public class Searcher {
	...
	public static final String DAUM_MAPS_LOCAL_KEYWORD_SEARCH_ANONYM_API_FORMAT = "https://apis.daum.net/local/v1/search/keyword.json?query=%s&location=%f,%f&page=%d&apikey=%s";
	...
	BK9 은행
	CT1 문화시설
	AG2 중개업소DAUM_MAPS_LOCAL_KEYWORD_SEARCH_ANONYM_API_FORMAT
	PO3 공공기관
	AT4 관광명소
	AD5 숙박
	FD6 음식점
	...
	private class SearchTask extends AsyncTask<String, Void, Void> {
		...
		protected Void doInBackground(String... urls) {
			...
			List<SearchModel> itemList = parse(json);
	...
	public void searchKeywordAnoymous(Context applicationContext, String query, double latitude, double longitude, int radius, int page, String apikey, OnFinishSearchListener onFinishSearchListener) {
		this.onFinishSearchListener = onFinishSearchListener;
		...
		String url = buildKeywordSearchApiUrlString(latitude,longitude, query, page, apikey);
		searchTask = new SearchTask();
		searchTask.execute(url);
	}
	...
	private String buildKeywordSearchApiUrlString(double latitude, double longitude, String query, int page, String apikey) {
		String encodedQuery = "";
		try {
			encodedQuery = URLEncoder.encode(query, "utf-8");
		} catch (UnsupportedEncodingException e) {
		...
		return String.format(DAUM_MAPS_LOCAL_KEYWORD_SEARCH_ANONYM_API_FORMAT, encodedQuery, latitude ,longitude, page, apikey);
	}
	...
	private List<SearchModel> parse(String jsonString) {
