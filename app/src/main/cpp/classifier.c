...
void predict_classifier2(char *datacfg, char *cfgfile, char *weightfile, char *filename, char *resource_path,  int top)
{
    network net = parse_network_cfg(cfgfile);
    if(weightfile){
        load_weights(&net, weightfile);
    ...
    //char *name_list = option_find_str(options, "names", 0);
//     char *name_list = strcat(resource_path, option_find_str(options, "names", 0)); // This is for imagenet
    if(!name_list) name_list = option_find_str(options, "labels", "data/labels.list");
...
float * run_classifier_custom(int argc, char **argv, int offset)
{
//    if(argc < 4){
...
//    }
    ...
    if(0==strcmp(argv[2], "predict2")){
        resource_path =argv[7];
    }
