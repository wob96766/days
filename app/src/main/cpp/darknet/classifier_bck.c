...
void predict_classifier2(char *resource_path, char *datacfg, char *cfgfile, char *weightfile, char *filename)
{
    network net = parse_network_cfg(cfgfile);
    if(weightfile){
        load_weights(&net, weightfile);
    ...
    char *name_list = strcat(resource_path, option_find_str(options, "names", 0));
    if(!name_list)
...
//    name_list = "/storage/emulated/0/Download/data/imagenet.shortnames.list";
...
void run_classifier(int argc, char **argv)
{
    ...
    if(0==strcmp(argv[2], "predict")) {
        resource_path = (argc > 7) ? argv[7] : 0;
        layer_s = (argc > 8) ? argv[8] : 0;
    }else{
    ...
    if(0==strcmp(argv[2], "predict")) predict_classifier2(resource_path, data, cfg, weights, filename);
